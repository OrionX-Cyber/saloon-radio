...
(function () {
  ...
  function renderTrackInfo(track) {
    ...
    if (el)  el.textContent  = track.title || 'தமிழ் பாடல்';
    if (mv)  mv.textContent  = track.movie || 'சலூன் ரேடியோ';
    if (img) img.src         = track.cover || ('https://i.ytimg.com/vi/' + track.youtubeId + '/hqdefault.jpg');
    document.title = (track.title || 'சலூன் ரேடியோ') + ' — Saloon Radio';
    updateSeekUI(0, 0);
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var playlistInfo = window.SaloonTracks.getPlaylistInfo();

    player.onTrackChange = renderTrackInfo;
    player.onStateChange = renderPlayState;

    if (playlistInfo.playlistId) {
      player.setYouTubePlaylist(playlistInfo.playlistId);
    } else {
      player.setPlaylist(playlistInfo.tracks);
